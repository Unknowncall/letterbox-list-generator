...
import logging
from jobs.sync_to_tmdb import run_sync_job, get_tmdb_config
...
async def trigger_tmdb_sync(
    ...
    **Errors:**
    ...
    if not config['enabled']:
        raise HTTPException(
            status_code=503,
            detail="TMDb sync is disabled. Set TMDB_SYNC_ENABLED=true in .env"
        )
    ...
    if not config['v4_access_token']:
        raise HTTPException(
            status_code=503,
            detail="TMDb v4 access token not configured. Set TMDB_V4_ACCESS_TOKEN in .env"
        )

    # Queue the sync job in background
    background_tasks.add_task(run_sync_job, request.usernames)

    logger.info(f"TMDb sync job queued for {len(request.usernames)} user(s): {', '.join(request.usernames)}")
