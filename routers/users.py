...
async def get_user_profile(
    username: str = Path(..., min_length=1, max_length=100, pattern=r'^[a-zA-Z0-9_-]+$', description="Letterboxd username")
):
    """
...
async def get_user_watchlist(
    ...
    sort_by: str = Query(default="title", pattern="^(title|year)$", description="Sort by: title or year (default: title)"),
    sort_order: str = Query(default="asc", pattern="^(asc|desc)$", description="Sort order: asc or desc (default: asc)")
):
    """
...
async def get_top_rated(
    ...
    page: int = Query(default=1, ge=1, description="Page number (default: 1)"),
    page_size: int = Query(default=20, ge=1, le=100, description="Number of films per page (default: 20)"),
    sort_by: str = Query(default="rating", pattern="^(rating|title|year)$", description="Sort by: rating, title, or year (default: rating)"),
    sort_order: str = Query(default="desc", pattern="^(asc|desc)$", description="Sort order: asc or desc (default: desc)")
):
    """
