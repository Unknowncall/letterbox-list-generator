...
async def get_user_watchlist(
    ...
    try:
        ...
        sort_key = None
        if sort_by == "title":
            sort_key = lambda x: (x.get('title') or "").lower()
        elif sort_by == "year":
            sort_key = lambda x: x.get('year') or 0
        ...
        raise ValueError(f"Error fetching watchlist: {str(e)}")
...
async def get_top_rated_films(
    ...
    try:
        ...
        sort_key = None
        if sort_by == "rating":
            sort_key = lambda x: x.get('rating', 0)
        elif sort_by == "title":
            sort_key = lambda x: (x.get('title') or "").lower()
        elif sort_by == "year":
            sort_key = lambda x: x.get('year') or 0
        ...
        raise ValueError(f"Error fetching top rated films: {str(e)}")
